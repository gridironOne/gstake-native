...
TEST_DOCKER_REPO=mkoijn6/gstakednode

HTTPS_GIT := https://github.com/gridironOne/gstake-native.git
DOCKER := $(shell which docker)
...
ifeq (cleveldb,$(findstring cleveldb,$(gSTAKE_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gstake \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gstaked \
...
ifeq (,$(findstring nostrip,$(gSTAKE_BUILD_OPTIONS)))
  ldflags += -w -s
endif
ldflags += $(LDFLAGS)
...
draw-deps:
	...
	@goviz -i ./cmd/gstaked -d 2 | dot -Tpng -o dependency-graph.png
...
build-docker-gstakednode:
	$(MAKE) -C networks/local

# Run a 4-node testnet locally
localnet-start: build-linux localnet-stop
	@if ! [ -f build/node0/gstaked/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/gstaked:Z mkoijn6/gstakednode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; fi
	docker-compose up -d
...
.PHONY: all build-linux install format lint \
	...
	benchmark \
	build-docker-gstakednode localnet-start localnet-stop \
	docker-single-node
