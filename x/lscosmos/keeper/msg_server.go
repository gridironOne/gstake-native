...
func (m msgServer) LiquidStake(goCtx context.Context, msg *types.MsgLiquidStake) (*types.MsgLiquidStakeResponse, error) {
	...
	protocolFee := hostChainParams.GstakeParams.GstakeDepositFee
	...
	if protocolCoin.IsPositive() {
		err = m.SendProtocolFee(ctx, sdktypes.NewCoins(protocolCoin), types.ModuleName, hostChainParams.GstakeParams.GstakeFeeAddress)
		if err != nil {
			return nil, errorsmod.Wrapf(
				types.ErrFailedDeposit, "failed to send protocol fee to gstake fee address %s, got error : %s",
				hostChainParams.GstakeParams.GstakeFeeAddress, err,
			)
		}
	}
	ctx.EventManager().EmitEvents(sdktypes.Events{
		sdktypes.NewEvent(
			...
			sdktypes.NewAttribute(types.AttributeGstakeDepositFee, protocolCoin.String()),
		),
		sdktypes.NewEvent(
...
func (m msgServer) LiquidUnstake(goCtx context.Context, msg *types.MsgLiquidUnstake) (*types.MsgLiquidUnstakeResponse, error) {
	...
	gstakeFeeAmt := hostChainParams.GstakeParams.GstakeUnstakeFee.MulInt(msg.Amount.Amount).TruncateInt()
	gstakeFee := sdktypes.NewCoin(msg.Amount.Denom, gstakeFeeAmt)
	if gstakeFeeAmt.IsPositive() {
		err = m.SendProtocolFee(ctx, sdktypes.NewCoins(gstakeFee), types.UndelegationModuleAccount, hostChainParams.GstakeParams.GstakeFeeAddress)
		if err != nil {
			return nil, err
		}
	...
	ctx.EventManager().EmitEvents(sdktypes.Events{
		sdktypes.NewEvent(
			...
			sdktypes.NewAttribute(types.AttributeGstakeUnstakeFee, gstakeFee.String()),
...
func (m msgServer) Redeem(goCtx context.Context, msg *types.MsgRedeem) (*types.MsgRedeemResponse, error) {
	...
	protocolCoin, _ := sdktypes.NewDecCoinFromDec(
		...
		hostChainParams.GstakeParams.GstakeRedemptionFee.MulInt(msg.Amount.Amount),
	).TruncateDecimal()
	...
	ctx.EventManager().EmitEvents(sdktypes.Events{
		sdktypes.NewEvent(
			...
			sdktypes.NewAttribute(types.AttributeGstakeRedeemFee, protocolCoin.String()),
		),
		sdktypes.NewEvent(
			sdktypes.EventTypeMessage,
			sdktypes.NewAttribute(sdktypes.AttributeKeyModule, types.AttributeValueCategory),
...
func (m msgServer) JumpStart(goCtx context.Context, msg *types.MsgJumpStart) (*types.MsgJumpStartResponse, error) {
	...
	hostChainParams := m.GetHostChainParams(ctx)
	if msg.GstakeAddress != hostChainParams.GstakeParams.GstakeFeeAddress {
		return nil, errorsmod.Wrap(sdkerrors.ErrInvalidAddress, fmt.Sprintf("msg.gstakeAddress should be equal to msg.GstakeParams.GstakeFeeAddress, got %s expected %s", msg.GstakeAddress, hostChainParams.GstakeParams.GstakeFeeAddress))
	}
	...
	// check fees limits
	if err := msg.GstakeParams.Validate(); err != nil {
		return nil, err
	}
	...
	newHostChainParams := types.NewHostChainParams(msg.ChainID, msg.ConnectionID, msg.TransferChannel,
		msg.TransferPort, msg.BaseDenom, msg.MintDenom, msg.GstakeParams.GstakeFeeAddress,
		msg.MinDeposit, msg.GstakeParams.GstakeDepositFee, msg.GstakeParams.GstakeRestakeFee,
		msg.GstakeParams.GstakeUnstakeFee, msg.GstakeParams.GstakeRedemptionFee)
	...
	ctx.EventManager().EmitEvents(sdktypes.Events{
		sdktypes.NewEvent(
			...
			sdktypes.NewAttribute(types.AttributeGstakeAddress, msg.GstakeAddress),
		),
		sdktypes.NewEvent(
			sdktypes.EventTypeMessage,
			sdktypes.NewAttribute(sdktypes.AttributeKeyModule, types.AttributeValueCategory),
			sdktypes.NewAttribute(sdktypes.AttributeKeySender, msg.GstakeAddress),
		)},
	)
...
func (m msgServer) ChangeModuleState(goCtx context.Context, msg *types.MsgChangeModuleState) (*types.MsgChangeModuleStateResponse, error) {
	...
	if hostChainParams.GstakeParams.GstakeFeeAddress != msg.GstakeAddress {
		return nil, errorsmod.Wrap(sdkerrors.ErrUnauthorized, fmt.Sprintf("Only admin address is allowed to call this method, current admin address: %s", hostChainParams.GstakeParams.GstakeFeeAddress))
	}
...
func (m msgServer) ReportSlashing(goCtx context.Context, msg *types.MsgReportSlashing) (*types.MsgReportSlashingResponse, error) {
	...
	if hostChainParams.GstakeParams.GstakeFeeAddress != msg.GstakeAddress {
		return nil, errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "Only admin address is allowed to call this method, current admin address: %s", hostChainParams.GstakeParams.GstakeFeeAddress)
	}
