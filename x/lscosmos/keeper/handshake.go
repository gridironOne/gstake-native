	...
	case sdk.MsgTypeURL(&banktypes.MsgSend{}):
		...
		if rewardAddress.Address == parsedMsg.FromAddress && delegationState.HostChainDelegationAddress == parsedMsg.ToAddress {
			...
			if amountOfBaseDenom.GT(sdk.ZeroInt()) {
				...
				gstakeFeeAmount := hostChainParams.GstakeParams.GstakeRestakeFee.MulInt(amountOfBaseDenom)
				protocolFee, _ := k.ConvertTokenToStk(ctx, sdk.NewDecCoinFromDec(hostChainParams.BaseDenom, gstakeFeeAmount), cValue)
				...
				err = k.SendProtocolFee(ctx, sdk.NewCoins(protocolFee), types.ModuleName, hostChainParams.GstakeParams.GstakeFeeAddress)
				if err != nil {
