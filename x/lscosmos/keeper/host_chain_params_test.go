...
func (suite *IntegrationTestSuite) TestHostChainParams() {
	...
	suite.Equal(GstakeFeeAddress, params.GstakeParams.GstakeFeeAddress)
	suite.Equal(sdk.NewInt(5), params.MinDeposit)
	suite.Equal(depositFee, params.GstakeParams.GstakeDepositFee)
	suite.Equal(restakeFee, params.GstakeParams.GstakeRestakeFee)
	suite.Equal(unstakeFee, params.GstakeParams.GstakeUnstakeFee)
}
